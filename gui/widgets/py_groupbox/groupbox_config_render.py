...
from gui.helpers.constants import UPDATE_TY_LE_KHUNG_HINH_VIDEO, CHANGE_HIEN_THI_SUB, PATH_OUTPUT, TypeTocDoChayAnh
...
class GroupBoxConfigRender(QWidget):
    ...
    def create_widgets(self):
        ...
        self.lb_edit_video = QLabel("Tùy Chỉnh Render")
    ...
    def setup_connections(self):
        ...
        #     lambda: self._checkboxChanged(self.checkbox_edit_video, "config_render"))
    ...
    def loadDataConfigCurrent(self, configCurrent: CauHinhTuyChonModel):
        ...
        if cau_hinh.get('toc_do_chay_hinh') is None:
            cau_hinh["toc_do_chay_hinh"] = 40
            self.configCurrent.value = json.dumps(cau_hinh)
            self.configCurrent.save()

        if cau_hinh.get('time_no_text') is None:
            cau_hinh["time_no_text"] = 3
            self.configCurrent.value = json.dumps(cau_hinh)
            self.configCurrent.save()

        if cau_hinh.get('speed_image_run') is None:
            cau_hinh["speed_image_run"] = TypeTocDoChayAnh.CO_DINH.value
            self.configCurrent.value = json.dumps(cau_hinh)
            self.configCurrent.save()
        ...
        try:
            bg_video_main = cau_hinh["bg_video_main"]
            bg_color_main = cau_hinh["bg_color_main"]
            bg_video_main = cau_hinh["src_bg_co_dinh"]
            bg_video_main = cau_hinh["sigma_bg_image_new"]
            bg_video_main = cau_hinh["sigma_bg_image_ori"]
        except:
            cau_hinh["bg_video_main"] = 'bg_image_origin'
            cau_hinh["bg_color_main"] = '#000000'
            cau_hinh["src_bg_co_dinh"] = ''
            cau_hinh["sigma_bg_image_new"] = 15
            cau_hinh["sigma_bg_image_ori"] = 15
            self.configCurrent.value = json.dumps(cau_hinh)
            self.configCurrent.save()
        ...
        if hasattr(self, "configCurrent"):
            ...
            if self.dialog_output_video.exec():
                ...
                if ti_le_khung_hinh != ti_le_khung_hinh_new or chat_luong_video_new != chat_luong_video:
                    self.manage_thread_pool.resultChanged.emit(UPDATE_TY_LE_KHUNG_HINH_VIDEO,
                                                               UPDATE_TY_LE_KHUNG_HINH_VIDEO, "")
            else:
                print("Cancel!")
        else:
