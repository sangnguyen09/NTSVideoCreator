...
from gui.helpers.constants import TEXT_TO_SPEECH_DEMO, JOIN_PATH, APP_PATH, UPDATE_BALANCE_API_TTS, CHECK_DO_LECH_VIDEO, \
    UPDATE_VALUE_PROGRESS_CHECK_LECH_SUB, \
    RESULT_CHECK_DO_LECH_VIDEO, CHECK_DO_LECH_PART_SUB, RESULT_CHECK_DO_LECH_VIDEO_PART, \
...
from gui.helpers.func_helper import remove_dir, play_media_preview, getValueSettings
...
from gui.widgets.py_combobox.py_checkable_combobox import MultiComboBox
...
class GroupBoxTextToSpeech(QWidget):
    def __init__(self, manage_thread_pool: ManageThreadPool, manage_cmd, groupbox_timeline, settings):
        ...
        self.EFFECT_VIDEO = settings.get("effect_video")
        ...
        tts_super_voice_new = {}
        tts_super_voice = self.list_gender_voice.get("tts_super_voice")
        for code, value in self.list_language_support.get('language_tts_super_voice').items():
            tts_super_voice_new.update({code: tts_super_voice})
        # print(tts_super_voice_new)
        self.list_gender_voice.update({"tts_super_voice": tts_super_voice_new})
    ...
    def create_widgets(self):
        self.groupbox = QGroupBox("Cấu Hình Lồng Tiếng")
        ...
        self.lb_mode_tts = QLabel("Hiệu Ứng Chuyển Động:")
        self.cb_effect_mode = MultiComboBox()
    ...
    def modify_widgets(self):
        ...
        self.cb_effect_mode.addItems(self.EFFECT_VIDEO.keys())
    ...
    def add_widgets_to_layouts(self):
        ...
        self.content_language_layout.addWidget(self.cb_person_voice, 10)
        ...
        # self.content_thuyet_minh.addWidget(QLabel())
        # self.content_language_layout.addWidget(QLabel())
        self.content_thuyet_minh.addWidget(self.lb_mode_tts)
        self.content_thuyet_minh.addWidget(self.cb_effect_mode, 10)
        self.content_thuyet_minh.addWidget(QLabel())
        self.content_thuyet_minh.addWidget(self.lb_channel_audio)
        self.content_thuyet_minh.addWidget(self.combo_channel_audio, alignment=Qt.AlignmentFlag.AlignLeft)
        self.content_thuyet_minh.addWidget(QLabel())
        ...
        self.content_status.addWidget(self.btn_demo, 15)
    ...
    def setup_connections(self):
        ...
        self.cb_effect_mode.comboboxTextChanged.connect(self.effectRenderChanged)
        ...
        self.cb_render_video_origin.stateChanged.connect(
            lambda: self._checkboxChanged(self.cb_render_video_origin, "use_video_origin"))
    ...
    def _openDialogFolderVoice(self):
        PyMessageBox().show_info("Lưu Ý",
                                 "Lưu ý Số thứ tự của từng dòng sub phải tương ứng với tên file nếu không sẽ bị lệch")
    ...
    def clickNgheThu(self):
        ...
        self.manage_thread_pool.start(self._funcNgheThu, "_funcNgheThu" + uuid.uuid4().__str__(), TEXT_TO_SPEECH_DEMO,
                                      gender=gender, language_tts=language_tts, style=style)
    ...
    def _funcNgheThu(self, **kwargs):
        ...
        file_temp = JOIN_PATH(PATH_VOICE, gender + "-" + style + "-" + language_tts + ".mp3")
        if os.path.exists(file_temp):
            play_media_preview(file_temp)
        else:
            if self.cbox_server.currentIndex() in [TabIndexTTS.TTSTiengViet.value, TabIndexTTS.TTSDaNgonNgu.value,
                                                   TabIndexTTS.SuperAIVoice.value]:
                user_data = getValueSettings(USER_DATA, TOOL_CODE_MAIN)
                headers = {"Authorization": f"Bearer {user_data['token']}"}
                response = requests.get(
                    url=URL_API_BASE + f"/tts/public/voice-demo/{gender}?style={style}&language={language_tts}",
                    headers=headers)
        ...
        for index_sub, sub in enumerate(data_sub):
            ...
            if index_sub < len(data_sub) - 1:
                ...
                start_sub_next = datetime.strptime(
                    next_sub[ColumnNumber.column_image.value].split(' --> ')[0], "%H:%M:%S,%f")
    ...
    def _resultThreadChanged(self, id_worker, id_thread, result):
        ...
        if id_thread == TTS_GET_VOICE_FINISHED:
            ...
            if isinstance(balance, dict):
                self.lb_blance.setText(
                    f"Ký Tự Còn Lại: {balance.get('characters') if isinstance(balance.get('characters'), str) else '{:,}'.format(balance.get('characters'))}")
            else:
                if not balance == -1:
        ...
        if id_thread == CHECK_DO_LECH_PART_SUB:
            if result != 'STOP':
                ...
                row_number, tempo, folder_temp = result
                remove_dir(folder_temp)
                self.manage_thread_pool.resultChanged.emit(RESULT_CHECK_DO_LECH_VIDEO_PART,
                                                           RESULT_CHECK_DO_LECH_VIDEO_PART, (
                                                               row_number, tempo))
        ...
        if id_thread == CHECK_DO_LECH_PART_SUMMARY:
            ...
            row_number, tempo, folder_temp = result
            remove_dir(folder_temp)
            self.manage_thread_pool.resultChanged.emit(RESULT_CHECK_DO_LECH_VIDEO_SUMMARY,
                                                       RESULT_CHECK_DO_LECH_VIDEO_SUMMARY, (
                                                           row_number, tempo))
        ...
        if id_thread == TEXT_TO_SPEECH_DEMO:
            ...
            if result == 'chua_nghe_duoc':
                ...
                return PyMessageBox().show_warning("Thông báo",
                                                   "Server hoặc Giọng đọc này hiện tại không lấy được voice")
    ...
    def loadDataConfigCurrent(self, configCurrent):
        ...
        try:
            mode = cau_hinh["effect_video"]

        except:
            cau_hinh["effect_video"] = ['bot']
            mode = cau_hinh["effect_video"]

            self.configCurrent.value = json.dumps(cau_hinh)
            self.configCurrent.save()
        temp = []
        for key, vl in self.EFFECT_VIDEO.items():
            if vl in mode:
                temp.append(key)

        self.cb_effect_mode.setChecked(temp)
    ...
    def getValue(self):
        ...
        tts = {"text_to_speech": self.groupbox.isChecked(),
               ...
               "effect_video": [self.EFFECT_VIDEO.get(eff) for eff in self.cb_effect_mode.currentData()],
    ...
    def effectRenderChanged(self):
        if hasattr(self, "configCurrent"):
            # print([ self.EFFECT_VIDEO.get(eff) for eff in self.cb_effect_mode.currentData()])
            if self.isClearData is False:
                cau_hinh = json.loads(self.configCurrent.value)
                cau_hinh["effect_video"] = [self.EFFECT_VIDEO.get(eff) for eff in self.cb_effect_mode.currentData()]
                self.configCurrent.value = json.dumps(cau_hinh)
                self.configCurrent.save()
