...
from PySide6.QtWidgets import QLabel, QLineEdit, QHBoxLayout, QVBoxLayout, QSpinBox, QGroupBox, QGridLayout, QDialog, \
	...
	QSlider, QWidget, QRadioButton

from gui.configs.config_resource import ConfigResource
from gui.configs.config_theme import ConfigTheme
from gui.helpers.constants import SETTING_APP_DATA, TOOL_CODE_MAIN, CHANGE_STYLE_DIALOG_ADD_SUB
...
class PyDialogGraphicConfigAddSub(QDialog):
	def __init__ (self, list_fonts, manage_thread_pool ):
		super().__init__()
		# LOAD SETTINGS
		# ///////////////////////////////////////////////////////////////
	...
	def create_widgets (self):
		self.groupbox_position_sub = QGroupBox("Vị Trí SUB gốc:")
		...
		self.rad_position_middlecenter = PyRadioButton(value=5, text="Middle Center")  # 5
		self.rad_position_middleleft = PyRadioButton(value=4, text="Middle Left")  # 4
	...
	def add_widgets_to_layouts (self):
		...
		self.content_layout.addWidget(self.groupbox_position_sub)
	...
	def setup_connections (self):
		...
		self.rad_position_bottomcenter.clicked.connect(self.checkRadioButtonPosition)
		self.rad_position_bottomleft.clicked.connect(self.checkRadioButtonPosition)
		self.rad_position_bottomright.clicked.connect(self.checkRadioButtonPosition)
		self.rad_position_middlecenter.clicked.connect(self.checkRadioButtonPosition)
		self.rad_position_middleleft.clicked.connect(self.checkRadioButtonPosition)
		self.rad_position_middleright.clicked.connect(self.checkRadioButtonPosition)
		self.rad_position_topcenter.clicked.connect(self.checkRadioButtonPosition)
		self.rad_position_topleft.clicked.connect(self.checkRadioButtonPosition)
		self.rad_position_topright.clicked.connect(self.checkRadioButtonPosition)
		self.rad_position_dragto.clicked.connect(self.checkRadioButtonPosition)
		#
		...
		self.groupbox_color.toggled.connect(lambda: self.dataConfigChanged(self.groupbox_color.isChecked(), 'add_mau_sub'))
		self.input_color.textChanged.connect(lambda: self.dataConfigChanged(self.input_color.text(), 'mau_sub'))
		self.slider_color_opacity.valueChanged.connect(lambda: self.dataConfigChanged(self.slider_color_opacity.value(), 'opacity_mau_sub'))
		
		self.groupbox_nen_chu.toggled.connect(lambda: self.dataConfigChanged(self.groupbox_nen_chu.isChecked(), 'nen_sub'))
		self.input_mau_nen.textChanged.connect(lambda: self.dataConfigChanged(self.input_mau_nen.text(), 'mau_nen_sub'))
		self.slider_mau_nen_opacity.valueChanged.connect(lambda: self.dataConfigChanged(self.slider_mau_nen_opacity.value(), 'opacity_nen_sub'))
		
		self.groupbox_font.toggled.connect(lambda: self.dataConfigChanged(self.groupbox_font.isChecked(), 'font_sub'))
		self.combobox_font_family.currentIndexChanged.connect(lambda: self.dataConfigChanged(self.combobox_font_family.currentText(), 'font_family'))
		self.combobox_font_size.currentIndexChanged.connect(lambda: self.dataConfigChanged(self.combobox_font_size.currentText(), 'font_size'))
		
	# self.groupBox_save_config.signalNameConfigChanged.connect(self.addConfigDB)
	
	def checkRadioButtonPosition (self):
		position_text = ""
		...
		cau_hinh['vi_tri_sub'] = position_text
		self.configCurrent.value = json.dumps(cau_hinh)
		self.configCurrent.save()
		self.manage_thread_pool.resultChanged.emit(CHANGE_STYLE_DIALOG_ADD_SUB, CHANGE_STYLE_DIALOG_ADD_SUB, "")
	...
	def dataConfigChanged (self, value, key_db):
		...
		if hasattr(self, 'configCurrent'):
			...
			self.configCurrent.value = json.dumps(cau_hinh)
			self.configCurrent.save()
			if key_db =='opacity_mau_sub':
				self.lb_opacity_color_value_current.setText(self.slider_color_opacity.value().__str__())
			elif key_db =='opacity_nen_sub':
				self.lb_opacity_nen_value_current.setText(self.slider_mau_nen_opacity.value().__str__())
			self.manage_thread_pool.resultChanged.emit(CHANGE_STYLE_DIALOG_ADD_SUB, CHANGE_STYLE_DIALOG_ADD_SUB, "")
	...
	def loadData (self, configCurrent):
		...
		# self.groupbox_style_sub.setChecked(cau_hinh["style_sub"])
		self.groupbox_color.setChecked(cau_hinh["add_mau_sub"])
		self.input_color.setText(cau_hinh["mau_sub"])
		self.slider_color_opacity.setValue(cau_hinh["opacity_mau_sub"])
		
		# self.groupbox_vien_chu.setChecked(cau_hinh["vien_sub"])
		# self.input_mau_vien.setText(cau_hinh["mau_vien_sub"])
		# self.combobox_do_day_vien.setCurrentText(cau_hinh["do_day_vien_sub"])
		self.groupbox_nen_chu.setChecked(cau_hinh["nen_sub"])
		self.input_mau_nen.setText(cau_hinh["mau_nen_sub"])
		self.slider_mau_nen_opacity.setValue(cau_hinh["opacity_nen_sub"])
		
		self.groupbox_font.setChecked(cau_hinh["font_sub"])
		self.combobox_font_family.setCurrentText(cau_hinh["font_family"])
		self.combobox_font_size.setCurrentText(cau_hinh["font_size"])
		
		self.color_picker.setCurrentColor(QColor(cau_hinh["mau_sub"]))
		# self.color_picker_mau_vien.setCurrentColor(QColor(cau_hinh["mau_vien_sub"]))
		self.color_picker_nen_chu.setCurrentColor(QColor(cau_hinh["mau_nen_sub"]))
		
		# self.groupbox_split_sub.setChecked(False if cau_hinh.get("split_sub_origin") is None else cau_hinh.get("split_sub_origin"))
		# self.max_character_inline.setValue(20 if cau_hinh.get("max_character_inline_origin") is None else cau_hinh.get("max_character_inline_origin"))
	...
	def checkRadioDisabled (self):
		...
		for rb in self.groupbox_position_sub.findChildren(QRadioButton):
			if rb.getValue() == cau_hinh["vi_tri_sub"]:
				rb.setChecked(True)
			# 
			# if is_all is True:
			# 	if cau_hinh['vi_tri_sub_dich'] in [4, 6, 7]:
			# 		if rb.getValue() in [4, 6, 7]:
			# 			rb.setDisabled(True)
			# 	
			# 	elif cau_hinh['vi_tri_sub_dich'] in [8, 10, 11]:
			# 		if rb.getValue() in [8, 10, 11]:
			# 			rb.setDisabled(True)
			# 	
			# 	elif cau_hinh['vi_tri_sub_dich'] in [1, 2, 3]:
	...
	def getValue (self):
		...
		return {
			"vi_tri_sub": position_text,
			# "style_sub": self.groupbox_style_sub.isChecked(),
			"add_mau_sub": self.groupbox_color.isChecked(),
			"mau_sub": self.input_color.text(),
			"opacity_mau_sub": self.slider_color_opacity.value(),
			"nen_sub": self.groupbox_nen_chu.isChecked(),
			"mau_nen_sub": self.input_mau_nen.text(),
			"opacity_nen_sub": self.slider_mau_nen_opacity.value(),
			"font_sub": self.groupbox_font.isChecked(),
			"font_family": self.combobox_font_family.currentText(),
