...
from gui.helpers.constants import ROW_SELECTION_CHANGED_TABLE_ADD_SUB, CHANGE_STYLE_SUB, \
    ...
    SETTING_APP_DATA, TOOL_CODE_MAIN, XOA_SUB_DONG_SO, THEM_KY_TU_NEU_SUB_CO_HAI_KYTU, PREVIEW_PRE_RENDER, \
    GET_VOICE_PREVIEW, \
    ...
    UPDATE_TABLE_TIMELINE_TEXT_SUB_CHANGED, ACTION_PRESS_CHECH_DOLECH
...
class TableTimelineAddSub(QWidget):
    ...
    def signal_get_voice(self):
        ...
        for index, item in enumerate(items_selected):
            ...
            Time, Text = self.getDataRow(item.row())
    ...
    def signal_split_sub(self):
        ...
        time_line = self.getValueItem(row_number, ColumnNumber.column_image.value)
    ...
    def signal_concat_sub(self):
        ...
        for index, item in enumerate(items_selected):
            ...
            time_line = self.getValueItem(row_start, ColumnNumber.column_image.value)
            start = time_line.split(' --> ')[0]
            end = time_line.split(' --> ')[1]
    ...
    def setup_table(self):
        ...
        self.main_table.horizontalHeader().setSectionResizeMode(ColumnNumber.column_image.value,
                                                                QHeaderView.ResizeMode.ResizeToContents)  # gian cách vừa với content
    ...
    def itemButtonAction(self, row_number, data):
        ...
        index2 = QPersistentModelIndex(self.main_table.model().index(row_number, ColumnNumber.column_image.value))
        ...
        index4 = QPersistentModelIndex(self.main_table.model().index(row_number, ColumnNumber.column_image.value))
    ...
    def checkDoLechPart(self):
        ...
        for index, item in enumerate(items_selected):
            ...
            image, text = self.getDataRow(item.row())
    ...
    def getDataSub(self):
        ...
        for row in range(self.model.rowCount()):
            ...
            file_image, text = self.getDataRow(row)
            # print(time, sub_origin, sub_translate, dolech, pos_ori, pos_trans)
            # data.append([time, sub_origin, sub_translate, pos_ori, pos_trans])
            data.append([file_image, text])
        return data
