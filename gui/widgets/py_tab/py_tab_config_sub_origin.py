...
from PySide6.QtWidgets import QWidget, QLabel, QLineEdit, QVBoxLayout, QGroupBox, QGridLayout, QColorDialog, QComboBox, \
...
class PyTabConfigSubOrigin(QWidget):
	...
	def checkValid (self):
		if self.groupbox_color.isChecked() is False and self.groupbox_nen_chu.isChecked() is False and self.groupbox_font.isChecked() is False:
			PyMessageBox().show_warning("Lỗi", "Thiếu dữ liệu bên TAB SUB GỐC Vui lòng điền đầy đủ")
			return False
	...
	def getValue (self):
		...
		return {
			...
			# "split_sub_origin": self.groupbox_split_sub.isChecked(),
			# "max_character_inline_origin": self.max_character_inline.value(),
			
			# "vien_sub": self.groupbox_vien_chu.isChecked(),
			# "mau_vien_sub": self.input_mau_vien.text(),
			# "do_day_vien_sub": self.combobox_do_day_vien.currentText(),
