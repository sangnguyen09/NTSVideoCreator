...
from gui.helpers.constants import ADD_TO_PROCESS, ADD_TO_TABLE_PROCESS, TRANSLATE_SUB_FINISHED, SET_STATUS_BUTTON_START, \
    TOOL_CODE_MAIN, USER_DATA, \
    START_RENDER_VIDEO_TTS, RENDER_VIDEO_FINISHED, \
    TOGGLE_SPINNER, JOIN_PATH, PATH_VIDEO, TEXT_TO_SPEECH_PUSH_PATH_AUDIO, PATH_AUDIO, PATH_LOGS, \
    THEM_KY_TU_NEU_SUB_CO_HAI_KYTU, XOA_SUB_DONG_SO, \
    LOAD_VIDEO_FROM_FILE_SRT, PREVIEW_PRE_RENDER, \
    UPDATE_STATUS_TABLE_PROCESS, GOP_SUB_DONG_TREN, \
    GOP_SUB_DONG_DUOI, THAY_THE_SUB_NEU_SUB_CO_DO_LECH_LON, LOAD_SUB_IN_TABLE_FINISHED, TypeBackgroundMainVideo, \
...
from ..py_table_widget.table_timeline_add_sub import TableTimelineAddSub
from ...configs.config_resource import ConfigResource
from ...helpers.custom_logger import customLogger
from ...helpers.func_helper import getValueSettings, has_audio
...
class PyTabAddSub(QWidget):
    def __init__(self, manage_thread_pool: ManageThreadPool, manage_cmd: ManageCMD, file_run_app, settings,
                 fileSRTCurrent, configCurrent):
        super().__init__()
        # PROPERTIES
    ...
    def create_widgets(self):
        ...
        self.groupbox_timeline = TableTimelineAddSub(self.manage_thread_pool)
        ...
        self.video_render = RenderVideo(self.manage_thread_pool, self.thread_pool_limit, self.manage_cmd,
                                        self.file_run_app, self.groupbox_timeline)
    ...
    def startThread(self):
        if self.folder_name and os.path.isdir(self.folder_name):
            ...
            if cau_hinh.get('bg_video_main') == TypeBackgroundMainVideo.BG_IMAGE_NEW.value:
    ...
    def _funcCheckErrorTTS(self, **kwargs):
        ...
        if 0 < len(text) < ky_tu_toi_thieu:
            return False, row_number, (
                "ERROR-TYPE-1\n- Dòng sub số {index} có nội dung dưới " + str(
                    ky_tu_toi_thieu) + " ký tự\n- Bấm THAY THẾ để thay lại nội dung\n- Bấm SAVE để tự động thêm dấu chấm cho đủ " + str(
                    ky_tu_toi_thieu) + " ký tự\n- Bấm DELETE để xóa",
                index, text)
    ...
    def _resultThreadChanged(self, id_worker, id_thread, result):
            ...
            if self.list_count_sub[(row_number)] == self.list_total_sub[(row_number)]:
                if len(self.list_sub_fail) > 0:
                    ...
                    for ind_row, err in sorted_dict_by_key.items():
                        ...
                        dial = PyDialogThoiGianSub(message.format(index=row_curr + 1), "Chỉnh sửa Dòng Sub", text_sub,
                                                   width=688, height=350)
                        if dial.exec():
                            if dial.is_delete:
                                count_delete += 1
                                ...
                                count_delete += 1
                                self.manage_thread_pool.resultChanged.emit(GOP_SUB_DONG_TREN, GOP_SUB_DONG_TREN,
                                                                           row_curr)

                            elif dial.is_bottom:
                                count_delete += 1
                                self.manage_thread_pool.resultChanged.emit(GOP_SUB_DONG_DUOI, GOP_SUB_DONG_DUOI,
                                                                           row_curr)

                            elif dial.is_auto_add:
                                self.manage_thread_pool.resultChanged.emit(THEM_KY_TU_NEU_SUB_CO_HAI_KYTU,
                                                                           THEM_KY_TU_NEU_SUB_CO_HAI_KYTU, (
                                                                               row_curr, '..'))
                            else:
                                self.manage_thread_pool.resultChanged.emit(THAY_THE_SUB_NEU_SUB_CO_DO_LECH_LON,
                                                                           THAY_THE_SUB_NEU_SUB_CO_DO_LECH_LON, (
                ...
                self.thread_pool_limit.start(self.funcRenderTTS, START_RENDER_VIDEO_TTS, START_RENDER_VIDEO_TTS,
                                             data=(
                                                 folder_audio, folder_video, list_fonts, rect_blur, list_layers,
                                                 folder_name,
        ...
        if id_thread == PREVIEW_PRE_RENDER:
            if hasattr(self, 'path_video'):
                ...
                self.video_render.render_video_preview(self.path_video, filters, row_number, cau_hinh, data_sub,
                                                       pos_add_sub)
    ...
    def funcRenderTTS(self, **kwargs):
        ...
        folder_audio, folder_video, list_fonts, rect_blur, list_layers, folder_name, row_number, cau_hinh, data_sub, data_sub_new = kwargs.get(
            'data')
        cau_hinh_edit = json.loads(self.fileSRTCurrent.value)
        pos_add_sub = cau_hinh_edit.get('pos_add_sub')
        self.manage_thread_pool.resultChanged.emit(UPDATE_PECENT_RANGE_TTS_CHUNK_TABLE_PROCESS,
                                                   UPDATE_PECENT_RANGE_TTS_CHUNK_TABLE_PROCESS,
                                                   {"row_number": row_number, "percent_range": 100 / 3})
        ...
        self.video_render.render_video_manga(folder_name, list_layers, list_fonts, rect_blur, row_number, cau_hinh,
                                             data_sub, data_sub_new, folder_audio, folder_video, pos_add_sub)
    ...
    def loadFileSRTCurrent(self, fileSRTCurrent, list_project, db_app, db_srt_file):
        self.fileSRTCurrent = fileSRTCurrent
        ...
        self.groupBox_load_sub.loadFileSRTCurrent(fileSRTCurrent, list_project, db_app, db_srt_file)
    ...
    def dropEvent(self, event):
        ...
        if len(lines) > 0:
            ...
            if os.path.isdir(folder_name):
                self.manage_thread_pool.resultChanged.emit(LOAD_VIDEO_FROM_FILE_SRT, LOAD_VIDEO_FROM_FILE_SRT,
                                                           folder_name)
            else:
