...
from gui.helpers.constants import DELIMITER_POS_SUB, DELIMITER_CENTER_POS_SUB
...
class ASS:
	...
	class Style:
		...
		formatters = None;
		order = ["Name", "Fontname", "Fontsize", "PrimaryColour", "SecondaryColour", "OutlineColour", "BackColour",
	...
	def replace_special (cls, text, space=" ", min_whitespace_length=1, max_whitespace_length=1):
		return cls.__re_remove_special.sub((
			lambda m: cls.__replace_special_replacer(m, space, min_whitespace_length, max_whitespace_length)), text);
	...
	def shiftscale (self, **kwargs):  # Shift/scale a section's geometry and/or timecodes
		...
		geometry_scale_origin = self.__kwarg_default(kwargs, "geometry_scale_origin", (
			0.0, 0.0));  # (x,y) geometry scaling origin
		...
		if (time_scale != 1.0 or time_offset != 0.0):
			self.__range_action(filter_types, start, end, full_inclusion, inverse, (
				lambda line: self.__shiftscale_action_time(inverse, split_naive, time_scale, time_scale_origin, time_offset, time_clip_start, time_clip_end, line)));
		...
		if (
			...
			used_styles = {};
			self.__range_action(filter_types, start, end, full_inclusion, inverse, (
				lambda line: self.__shiftscale_action_geometry(geometry_scale, geometry_scale_origin, geometry_offset, resolution_old, resolution_new, bounds, used_styles, line)));
	...
	def __shiftscale_action_geometry (self, geometry_scale, geometry_scale_origin, geometry_offset, resolution_old, resolution_new, bounds, used_styles, line):
		...
		# Modify bounds
		ml, mr, mv = self.__shiftscale_action_get_new_margins(bounds, geometry_scale, resolution_new,
			state["align"], line.Style, line.MarginL, line.MarginR, line.MarginV);
		line.MarginL = ml;
		line.MarginR = mr;
		line.MarginV = mv;
	...
	def __shiftscale_action_geometry_modify_tag (self, state, geometry_scale, geometry_scale_origin, geometry_offset, tag):
		...
		if (tag_name in ["bord", "shad", "be", "blur", "fs"]):
			...
			tag = [
				tag_name,
				self.Formatters.number_to_str((self.Formatters.str_to_number(tag[1]) - geometry_scale_origin[0]) *
											  geometry_scale[0] + geometry_scale_origin[0] + geometry_offset[0]),
				self.Formatters.number_to_str((self.Formatters.str_to_number(tag[2]) - geometry_scale_origin[1]) *
											  geometry_scale[1] + geometry_scale_origin[1] + geometry_offset[1]),
			];
			...
			if (len(tag) == 5):
				...
				for i in range(1, len(tag)):
					...
					val = (val - geometry_scale_origin[xy]) * geometry_scale[xy] + geometry_scale_origin[xy] + \
						  geometry_offset[xy];
				...
				tag = list(tag);
				tag[
					-1] = self.__shiftscale_action_geometry_modify_geometry(geometry_scale, geometry_scale_origin, geometry_offset,
					tag[-1]);
		elif (tag_name == "move"):
			tag = list(tag);
			for i in range(1, len(tag)):
			...
			# Parse more tags
			tag[-1] = self.parse_tags(tag[-1], modify_tag=(
				lambda tag2: self.__shiftscale_action_geometry_modify_tag(None, geometry_scale, geometry_scale_origin, geometry_offset, tag2)));
	...
	def __shiftscale_action_geometry_modify_geometry (self, geometry_scale, geometry_scale_origin, geometry_offset, geo):
		...
		for i in range(len(points)):
			...
			val = (val - geometry_scale_origin[xy]) * geometry_scale[xy] + geometry_scale_origin[xy] + geometry_offset[
				xy];
	...
	def remove_formatting (self, **kwargs):  # Remove special formatting from lines
		...
		# Modify lines
		self.__range_action(filter_types, start, end, full_inclusion, inverse, (
			lambda line: self.__remove_formatting_action(modify_text, modify_tag_block, modify_tag, modify_comment, modify_geometry, line)));
...
def create_file_sub_ass (filename_srt, cau_hinh, sequences, pos_add_sub):
	# print(cau_hinh["chat_luong_video"])
	try:
		output_size = cau_hinh["chat_luong_video"]
		width_V, height_V = output_size.split("|")
		# pos_ori = cau_hinh.get("pos_add_sub")
	except:
		width_V, height_V = 1920, 1080
	# print(width_V, height_V)
	if int(width_V) > int(height_V):
		ratio = int(width_V) / int(height_V)
	
	else:
		ratio = int(height_V) / int(width_V)
	
	source = [
		"[Script Info]\n",
		"; Script generated by NTSautosub\n",
		"ScriptType: v4.00+\n",
		f"PlayResX: {width_V}\n",
		f"PlayResY: {height_V}\n",
	...
	font_name = cau_hinh['font_family'] if cau_hinh['font_sub'] is True else 'Arial'
	font_size = cau_hinh['font_size'] if cau_hinh['font_sub'] is True else '50px'
	font_size = int(int(font_size.replace('px', '')))
	# print(font_size)
	# print(ratio)
	PrimaryColour = color_to_hex(cau_hinh['mau_sub'], cau_hinh['opacity_mau_sub'])
	OutlineColour = '&H00000000'
	BackColour = color_to_hex(cau_hinh['mau_nen_sub'], cau_hinh['opacity_nen_sub']) if cau_hinh[
																						   'nen_sub'] is True else '&H00000000'
	BorderStyle = 4 if cau_hinh['nen_sub'] is True else 1
	Outline = 0
	Alignment = cau_hinh['vi_tri_sub']
	...
	# if type_srt == 'trans':
	# 	# print('Trans')
	# 	font_name = cau_hinh['font_family_sub_dich'] if cau_hinh['font_sub_dich'] is True else 'Arial'
	# 	font_size = cau_hinh['font_size_sub_dich'] if cau_hinh['font_sub_dich'] is True else '50px'
	# 	font_size = int(font_size.replace('px', ''))
	# 	PrimaryColour = color_to_hex(cau_hinh['mau_sub_dich'], cau_hinh['opacity_mau_sub_dich'])
	# 	OutlineColour = '&H00000000'
	# 	BackColour = color_to_hex(cau_hinh['mau_nen_sub_dich'], cau_hinh['opacity_nen_sub_dich']) if cau_hinh[
	# 																									 'nen_sub_dich'] is True else '&H00000000'
	# 	BorderStyle = 4 if cau_hinh['nen_sub_dich'] is True else 1
	# 	Outline = 0
	# 	Alignment = cau_hinh['vi_tri_sub_dich']
	# 	MarginL = 10
	# 	MarginR = 10
	# 	MarginV = 10
	source.append("Format: Name, Fontname, Fontsize, PrimaryColour, SecondaryColour, OutlineColour, BackColour, Bold, Italic, Underline, StrikeOut, ScaleX, ScaleY, Spacing, Angle, BorderStyle, Outline, Shadow, Alignment, MarginL, MarginR, MarginV, Encoding\n")
	
	source.append(f"Style: Default,{font_name},{font_size},{PrimaryColour},&H00FFFFFF,{OutlineColour},{BackColour},0,0,0,0,100,100,0,0,{BorderStyle},{Outline},0,{Alignment},{MarginL},{MarginR},{MarginV},1")
	# print(source)
	source.append(u"\n\n")
	...
	for seq in sequences:
		...
		#
		# 	try:
		# 		x, y = pos_trans.split(DELIMITER_POS_SUB)
		# 		pos = "{\pos(" + f"{x},{y}" + ")}"
		# 	except:
		# 		try:
		# 			x, y = pos_trans.split(DELIMITER_CENTER_POS_SUB)
		# 			center_pos = '\\an5'
		# 			# pos = "{\pos(" + f"{x},{y}" + ")}"
		# 		except:
		# 			# pos = ''
		...
		# 			x, y = False, False
		# 	if x and y:
		# 		pos = "{\pos(" + f"{x},{y}" + f")\\fsp1{center_pos}\q0\\fs{font_size}\\fscx100\\fscy100" + "}"
		# 	else:
		# 		pos =''
		#
		# else:
		...
		try:
			x, y = pos_add_sub.split(DELIMITER_POS_SUB)
		except:
			try:
				x, y = pos_add_sub.split(DELIMITER_CENTER_POS_SUB)
				center_pos = '\\an5'
		...
		MarginV = 10
		text = text.replace('\n', '\\N')
		source.append(f"Dialogue: 0,{start_},{end_},Default,,{MarginL},{MarginR},{MarginV},,{pos}{text}\n")
	
	# Write file
	file_data = open(filename_srt, 'w', encoding='utf-8')
	file_data.write((u"".join(source)))
	file_data.close()


def str_to_timecode (val, decimal_length=2, seconds_length=2, minutes_length=2, hours_length=1, *args):
	time = 0.0
	mult = 1
	...
	tc_parts = [
		"{{0:0{0:d}d}}".format(hours_length).format(int(time // 3600)),
		"{{0:0{0:d}d}}".format(minutes_length).format(int((time // 60) % 60)),
		"{{0:0{0:d}.{1:d}f}}".format(total_length, decimal_length).format(time % 60),
	]
	return ":".join(tc_parts)


def color_to_hex (color, alpha, *args):
	match = re.compile(r"([a-fA-F0-9]{1,8})", re.U).search(color)
	if (match):
		hex_val = "{0:>08s}".format(match.group(1))
		return "&H{0:02X}{1:02X}{2:02X}{3:02X}".format(int(255 * (100 - alpha) / 100), int(hex_val[
																						   6: 8], 16), int(hex_val[
																										   4: 6], 16), int(hex_val[
																														   2: 4], 16))
	else:
		return "&H{0:02X}{1:02X}{2:02X}{3:02X}".format(0, 255, 255, 255)


if __name__ == "__main__":
	print(color_to_hex("#550000", 68))
	mmm ={"1":"Dsd", "2":"Dsd", "3":"Dsd"}
	print(len(mmm))