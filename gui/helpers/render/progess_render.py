...
class RenderVideo(QWidget):
    ...
    def render_video_image(self, folder_name, list_layers, list_fonts, rect_blur, row_number, cau_hinh, data_sub,
                           data_sub_new, folder_audio, folder_video, pos_add_sub):
        pass

    # chuyển Hình Thành Video
    def render_video_TTS_split(self, video_path, list_layers, list_fonts, rect_blur, file_output, row_number, sub_path,
                               cau_hinh, data_sub, data_sub_new, folder_audio, folder_video,
                               pos_add_sub):  # lấy cấu hinh ngay lúc bấm để kiểm tra mới đùng
        ...
        if cau_hinh.get("use_video_origin"):
            ...
            if res.json()["status_code"] == 200:
                ...
                self.thread_pool_limit.start(self._funcRenderPreTTS, video_path,
                                             RENDER_VIDEO_PRE_TTS, limit_thread=True, data_res=res.json()["data"],
                                             paes=user_data[
                                                 'paes'], row_number=row_number, total_seconds=range_time_ms,
                                             file_srt_origin=file_srt_origin, file_output=file_output,
                                             list_file_temp=list_file_temp, name_video=name_video, cau_hinh=cau_hinh)
    ...
    def _funcRenderPreTTS(self, **kwargs):
        ...
        show_log = cau_hinh.get('show_log')
                                          ...
                                          RESULT_CMD_RENDER_VIDEO_PRE_TTS, logs=True, print=show_log,
                                          use_stop_thread=True, row_number=row_number, total_seconds=total_seconds,
                                          file_srt_origin=file_srt_origin, file_output=file_output,
    ...
    def func_r_v_l(self, **kwargs):
        ...
        try:
            ...
            if ind_seq == 0 and second_start > 0.3:
                ...
                if checkVideoOk(file_start) is False:
                    ff_code = list_commands.get("start").format(video_path=video_path, second_start=second_start,
                                                                file_start_temp=file_start_temp)
                else:
                ...
                if list_commands.get('start_volume') is not None:
                    ff_code = list_commands.get("start_volume").format(file_start_temp=file_start_temp,
            ...
            while True:
                ...
                if checkVideoOk(file_output) is False:
                    # Cắt video
                    ff_code = list_commands.get(f"strim").format(start_time=start_time, video_path=video_path,
                                                                 seconds_sub=seconds_sub, file_strim=file_strim)
                else:
                    ff_code = f'ffmpeg -version'
                # print(ff_code)
                            ...
                            if duration_voice > 0:
                                if seconds_sub == 0:
                                    ...
                                    if tempo == 0:
                                        ...
                                        self.manage_thread_pool.resultChanged.emit(STOP_GET_VOICE, STOP_GET_VOICE,
                                                                                   row_number)
                                        self.manage_thread_pool.messageBoxChanged.emit("Cảnh báo",
                                                                                       f"Dòng sub {ind_seq + 1} không get được voice",
                                                                                       "warning")
                                        return False, row_number, ''
                                ...
                                if tempo < 1:
                                    if mode_tts == 'v3':
                                        ...
                                        # 	atempo = f"atempo={atempo__}"
                                        # print(atempo)
                                        ff_code = list_commands.get(f"mix_<1").format(atempo=atempo, tempo=str(tempo),
                                                                                      file_strim=file_strim,
                                                                                      file_audio=file_audio,
                                                                                      seconds_sub=seconds_sub,
                                                                                      file_output=file_output,
                                                                                      end_time=duration_voice)
                                    ...
                                    ff_code = list_commands.get(f"mix_>=1").format(atempo=atempo, tempo=str(tempo),
                                                                                   ...
                                                                                   file_audio=file_audio,
                                                                                   seconds_sub=seconds_sub,
                                                                                   end_time_sub=duration_voice,
                                                                                   file_output=file_output)
                                # print(atempo)
                                # print(ff_code)
                                self.manage_cmd.cmd_output(ff_code,
                                                           RENDER_VIDEO_TTS_GHEP_VOICE_COMMAND + f'_addsub_{ind_seq + 1}',
                                                           RENDER_VIDEO_TTS_GHEP_VOICE_COMMAND, logs=True,
                                                           print=show_log, row_number=row_number)
        ...
        self.manage_thread_pool.resultChanged.emit(RENDER_VIDEO_TTS_GHEP_VOICE_COMMAND,
                                                   ...
                                                   (True, row_number, list_video_output))
        ...
        if id_thread == RENDER_VIDEOPRE_FINISHED:
            ...
            if res.status_code == 200:
                ...
                for ind_seq, sub in enumerate(data_sub):
                    ...
                    self.thread_limit_tts_chunk.start(self.func_r_v_l, RENDER_VIDEO_TTS_GHEP_VOICE_COMMAND,
                                                      RENDER_VIDEO_TTS_GHEP_VOICE_COMMAND,
                                                      data_thread=(
                ...
                self.manage_thread_pool.resultChanged.emit(RENDER_VIDEO_FFMPEG_TTS_CHUNK_ERROR,
                                                           RENDER_VIDEO_FFMPEG_TTS_CHUNK_ERROR,
                                                           (False, 423, row_number))
        ...
        if id_thread == RENDER_VIDEO_TTS_GHEP_VOICE_COMMAND:
            ...
            self.manage_thread_pool.progressChanged.emit(UPDATE_VALUE_PROGRESS_TTS_CHUNK_TABLE_PROCESS, json.dumps({
                "row_number": row_number, "index_chunk": 3}),
                                                         self.list_count_chunk_finished[
                                                             (row_number)])  # index_chunk 3 là ghép voice

            print("Tổng Sub: ",
                  self.list_total_chunks[(row_number)], " Đã Render Được: ",
                  self.list_count_chunk_finished[(row_number)])
        ...
        if id_thread == Noi_Video_theo_chunk:
            ...
            self.manage_thread_pool.start(self.func_concat_list, 'CONCAT_VIDEO_FILE_LIST1',
                                          'CONCAT_VIDEO_FILE_LIST1', data=(ls_video_input,
                                                                           row_number, folder_video, folder_audio,
                                                                           file_concat_video,
    ...
    def func_concat_list(self, **kwargs):
        ls_video_input, row_number, folder_video, folder_audio, file_concat_video, file_output_finished, cau_hinh, list_commands = kwargs.get(
            'data')
        ...
        self.manage_cmd.cmd_output(ffcode, CONCAT_VIDEO_FILE_LIST, CONCAT_VIDEO_FILE_LIST, logs=True,
                                   folder_audio=folder_audio, folder_video=folder_video,
                                   file_output=file_output_finished, file_concat_video=file_concat_video,
                                   range_time_ms=range_time_ms, row_number=row_number, cau_hinh=cau_hinh)
        ...
        if type_cmd == CONCAT_VIDEO_FILE_LIST:
            ...
            if cau_hinh['them_nhac'] is True or (
                ...
                if cau_hinh.get("smart_cut"):
                    ...
                    self.manage_thread_pool.start(self.fn_smart_cut, SMART_CUT_VIDEO_HAS_MUSIC,
                                                  SMART_CUT_VIDEO_HAS_MUSIC, data=(
                    ...
                    self.xu_ly_nhac_nen(file_output, cau_hinh, file_concat_video, range_time_ms, list_commands,
                                        row_number, folder_audio, folder_video)
    ...
    def fn_smart_cut(self, **kwargs):
        ...
        # print(cmd)
        self.manage_thread_pool.resultChanged.emit(RESET_VALUE_PROGRESS_TABLE_PROCESS,
                                                   RESET_VALUE_PROGRESS_TABLE_PROCESS, row_number)
        self.manage_thread_pool.statusChanged.emit(str(row_number), UPDATE_STATUS_TABLE_PROCESS,
                                                   "Chuẩn Bị Cắt Thông Minh")
        self.manage_cmd.cmd_output(cmd, CMD_SMART_CUT_VIDEO, CMD_SMART_CUT_VIDEO, print=False,
                                   video_ouput=file_output_cut, row_number=row_number)
        if checkVideoOk(file_output_cut):
            return file_output_cut, *kwargs.get('data_return', ())

    def xu_ly_nhac_nen(self, file_output, cau_hinh, file_concat_video, range_time_ms, list_commands, row_number,
                       folder_audio, folder_video):
        ''' Xử lý Nhạc Nền '''
        ffmpeg = ffmpeg_check()
        ...
        ffcode = list_commands.get("music").format(video_input=file_concat_video, file_music=file_music,
        ...
        self.manage_cmd.execute(ffmpeg, ffcode, CONCAT_VIDEO_FINAL, CONCAT_VIDEO_FINAL, logs=True,
                                folder_audio=folder_audio, folder_video=folder_video, row_number=row_number,
                                file_output=file_output, range_time_ms=range_time_ms)
