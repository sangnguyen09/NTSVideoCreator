...
from gui.helpers.translatepy.exceptions import RequestStatusError
...
class Request():
	def __init__ (self, proxy_urls: Union[str, List] = None, cache_duration: Union[int, float] = 2):
		...
		self.proxies = (
			[proxy_urls] if isinstance(proxy_urls, str) else list(proxy_urls) if proxy_urls is not None else [])
