...
from gui.helpers.translatepy.exceptions import ParameterTypeError, ParameterValueError, TranslatepyException, \
	UnsupportedMethod, UnsupportedLanguage
from gui.helpers.translatepy.language import Language
from gui.helpers.translatepy.models import (DictionaryResult, ExampleResult,
											LanguageResult, SpellcheckResult,
											TextToSpechResult, TranslationResult,
											TransliterationResult)
from gui.helpers.translatepy.utils.annotations import List
from gui.helpers.translatepy.utils.lru_cacher import LRUDictCache
from gui.helpers.translatepy.utils.sanitize import remove_spaces
...
class BaseTranslator(ABC):
	...
	_text_to_speeches_cache = LRUDictCache()
	
	_supported_languages = {}
	
	def translate (self, text: str, destination_language: str, source_language: str = "auto", **kwargs) -> TranslationResult:
		"""
		...
		if _cache_key in self._translations_cache:
			...
			source_language, translation = self._translate(text, dest_code, source_code, **kwargs)
			
			# Cache the translation values to speed up the translation process in the future
			if not translation in ["", b'', None]:
	...
	def translate_html (self, html: Union[
		str, PageElement, Tag, BeautifulSoup], destination_language: str, source_language: str = "auto", parser: str = "html.parser", threads_limit: int = 100) -> \
			Union[str, PageElement, Tag, BeautifulSoup]:
		"""
		...
		nodes = [tag for tag in page.find_all(text=True, recursive=True) if
				 not isinstance(tag, (PreformattedString)) and remove_spaces(tag) != ""]
	...
	def text_to_speech (self, text: str, speed: int = 100, gender: str = "female", source_language: str = "auto", **kwargs) -> TextToSpechResult:
		"""
		...
		#
		# Build cache key
		_cache_key = str({"t": text, "sp": speed, "s": source_language, "g": gender, "st": kwargs.get('style')})
		...
		if _cache_key in self._text_to_speeches_cache:
			...
			source_language, text_to_speech = self._text_to_speech(text, speed, gender, source_language, **kwargs)
			if isinstance(text_to_speech, bytes) and not text_to_speech in ["", b'', None, False]:
	...
	def _text_to_speech (self, text: str, speed: int, gender: str, source_language: str, **kwargs) -> bytes:
		"""
		Private method that concrete Translators must implement to hold the concrete
