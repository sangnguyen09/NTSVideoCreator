...
from gui.helpers.translatepy.utils.annotations import Dict
...
class BingSessionManager():
    def __init__(self, request: Request, captcha_callback: Callable[[str], str] = None):
        ...
        _auth_session_data = {}
        self.ig, self.iid, self.key, self.token, self.cookies = _auth_session_data.get("id"), _auth_session_data.get(
            "iid"), _auth_session_data.get("key"), _auth_session_data.get("token"), _auth_session_data.get("cookies")
        self.captcha_callback = captcha_callback
        # print(_auth_session_data)
    ...
    def _parse_authorization_data(self):
        for _ in range(3):
            ...
            raise BingTranslateException(
                message="Can't parse the authorization data, try again later or use MicrosoftTranslate")
...
class BingTranslate(BaseTranslator):
    ...
    _supported_languages = {'auto-detect', 'af', 'sq', 'am', 'ar', 'hy', 'as', 'az', 'bn', 'bs', 'bg', 'my', 'ca', 'ca',
                            'zh-Hans', 'cs', 'da', 'nl', 'nl', 'en', 'et', 'fj', 'fil', 'fil', 'fi', 'fr', 'fr-ca',
                            'de', 'ga', 'el', 'gu', 'ht', 'ht', 'he', 'hi', 'hr', 'hu', 'is', 'iu', 'id', 'it', 'ja',
                            'kn', 'kk', 'km', 'ko', 'ku', 'lo', 'lv', 'lt', 'ml', 'mi', 'mr', 'ms', 'mg', 'mt', 'ne',
                            'nb', 'nb', 'or', 'pa', 'pa', 'fa', 'pl', 'pt', 'ps', 'ps', 'ro', 'ro', 'ro', 'ru', 'sk',
                            'sl', 'sm', 'es', 'es', 'sr-Cyrl', 'sw', 'sv', 'ty', 'ta', 'te', 'th', 'ti', 'tlh-Latn',
                            'tlh-Latn', 'to', 'tr', 'uk', 'ur', 'vi', 'cy', 'zh-Hans', 'zh-Hant', 'yue', 'prs', 'mww',
                            'tlh-Piqd', 'kmr', 'pt-pt', 'otq', 'sr-Cyrl', 'sr-Latn', 'yua'}

    def __init__(self, request: Request = Request()):
        self.session_manager = BingSessionManager(request)
        self.session = request

    def _translate(self, text: str, destination_language: str, source_language: str,**kwargs) -> str:
        response = self.session_manager.send("https://www.bing.com/ttranslatev3",
                                             data={'text': text, 'fromLang': source_language,
    ...
    def _example(self, text, destination_language, source_language) -> str:
        ...
        response = self.session_manager.send("https://www.bing.com/texamplev3",
                                             data={'text': text.lower(), 'from': source_language,
                                                   'to': destination_language, 'translation': translation.lower()})
    ...
    def _spellcheck(self, text: str, source_language: str) -> str:
        ...
        response = self.session_manager.send("https://www.bing.com/tspellcheckv3",
                                             data={'text': text, 'fromLang': source_language})
    ...
    def _dictionary(self, text: str, destination_language: str, source_language: str):
        ...
        response = self.session_manager.send("https://www.bing.com/tlookupv3",
                                             data={'text': text, 'from': source_language, 'to': destination_language})
        _result = []
    ...
    def _text_to_speech(self, text: str, speed: int, gender: str, source_language: str,**kwargs):
        raise BingTranslateException(status_code=719,
                                     message="DEPRECATED! Use Microsoft Translate's text to spech method")
