...
class TTSTiengVietTranslator(BaseTranslator):
	...
	def check_balance (self):
		...
		response = requests.post(url=URL_API_BASE + "/tts/private/check-balance/tieng-viet", headers=headers, json={
	...
	def _text_to_speech (self, text: str, speed: int, gender: str, source_language: str, **kwargs):
		...
		for i in range(20):
			if self.tts_fail.get(kwargs.get('row_number', 0)):
				raise Exception
			try:
				response = requests.post(url=URL_API_BASE + "/tts/private/tieng-viet", headers=headers, json={
					"data": data_encrypt})
			except:
				continue
			if response.status_code == 200:
