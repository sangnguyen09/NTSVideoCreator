...
class TTSDaNgonNguTranslator(BaseTranslator):
	...
	def check_balance (self):
		...
		response = requests.post(url=URL_API_BASE + "/tts/private/check-balance/da-ngon-ngu", headers=headers, json={
	...
	def _text_to_speechv1 (self, text: str, speed: int, gender: str, source_language: str, **kwargs):
		...
		for i in range(5):
			response = requests.post(url=URL_API_BASE + "/tts/private/da-ngon-ngu", headers=headers, json={
				"data": mh_ae(payload, p_k=self.user_data['paes'])})
			if response.status_code == 200:
