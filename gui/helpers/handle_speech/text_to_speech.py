...
from gui.db.sqlite import ConfigApp_DB, CauHinhTuyChonModel
...
from gui.helpers.constants import PATH_AUDIO, JOIN_PATH, PATH_VIDEO, CHECK_LIVE_PROXY_ON_TTS, \
	...
	RENDER_VIDEOPRE_FINISHED, TEXT_TO_SPEECH_FINISHED, GET_VOICE_PREVIEW, APP_PATH, TTS_GET_VOICE_FINISHED
from gui.helpers.func_helper import get_duaration_video, ffmpeg_check, pitch_shift, check_exsist_app, remove_dir
from gui.helpers.http_request.check_proxy import ProxyChecker
from gui.helpers.render.progess_render import RenderVideo
from gui.helpers.server import SERVER_TAB_TTS
from gui.helpers.thread import ManageThreadPool, ManageCMD
from gui.helpers.translatepy.translators import BaseTranslator
...
class TextToSpeech():
	...
	def getVoiceRow (self, cau_hinh, sub_tss, index, row_number, folder_audio):
		...
		self._funcTextToSpeechChunk(server_trans=server_trans, row_number=row_number, get_voice=True,
			index=index, text=sub_tss, gender=gender, style=style, language_tts=language_tts, pitch=pitch, folder_audio=folder_audio, cau_hinh=cau_hinh)
	...
	def convertTextToSpeechChunk (self, row_number, cau_hinh, data_sub, folder_audio, get_voice=False, list_rows=None):
		...
		if not get_voice:
			self.manage_thread_pool.resultChanged.emit(UPDATE_RANGE_PROGRESS_TTS_CHUNK_TABLE_PROCESS,
				UPDATE_RANGE_PROGRESS_TTS_CHUNK_TABLE_PROCESS,
				{"row_number": row_number, "range": len(data_sub), "index_chunk": 1})
		...
		for index, sub in enumerate(data_sub):
			...
			self.thread_pool_limit.start(self._funcTextToSpeechChunk, "text_to_speech_" + str(index + 1),
				...
				index=index, text=sub_tss, gender=gender, style=style, language_tts=language_tts, pitch=pitch, folder_audio=folder_audio, folder_temp=folder_temp, cau_hinh=cau_hinh)
	...
	def _funcTextToSpeechChunk (self, **kwargs):
		...
		if "server_trans" in kwargs.keys():
			...
			if not os.path.exists(file_out) or get_voice:
				...
				text_trans = server_trans.text_to_speech(text, 100, gender, language_tts, row_number=row_number, style=style, folder_src_voice=cau_hinh.get("folder_src_voice", ""), line_sub=str(index + 1))
				...
				try:  # loại bỏ khoảng lạng đầu và cuối
					...
					self.manage_cmd.cmd_output(code_ff, str(row_number) + TRIM_AUDIO_SLICE + str(index + 1), TRIM_AUDIO_SLICE)
	...
	def _resultThread (self, id_worker, id_thread, result):
		...
		if id_thread == TEXT_TO_SPEECH_CHUNK:
			...
			if is_ok is False:
				...
				PyMessageBox().show_warning("Thông báo", "Server lấy giọng đọc hiện tại đang chậm hoặc bị quá tải không lấy được voice")
			elif is_ok == 'STOP':
				print('stop')
				return
			else:
				...
				if self.count_result_tts[(row_number)] == self.total_sub[(row_number)]:
					...
					self.manage_thread_pool.resultChanged.emit(TEXT_TO_SPEECH_FINISHED, TEXT_TO_SPEECH_FINISHED, row_number)
					if get_voice:
						self.manage_thread_pool.resultChanged.emit(TTS_GET_VOICE_FINISHED, TTS_GET_VOICE_FINISHED, file_out)
	...
	def getFolderAudioRow (self, folder_name):
		...
		if os.path.isdir(path):
			is_ok = PyMessageBox().show_question("Thông báo", "Đã tồn tại trạng thái chuyển thành voice trước đó với video này. Bạn có muốn chuyển lại voice không ? Chuyển lại sẽ làm lãng phí ký tự của bạn.")
			if is_ok:
