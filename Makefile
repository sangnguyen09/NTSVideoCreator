...
bdc:
	...
	python -m nuitka --no-deployment-flag=self-execution --onefile --msvc=14.3 --enable-plugin=data-hiding --enable-plugin=anti-debugger  --remove-output --noinclude-numba-mode=nofollow --include-data-files=E:/Project/Python/NTSVideoCreator/libsndfile.dll=.  --include-package-data=budoux --include-package-data=tls_client --include-package-data=undetected_chromedriver --include-package-data=seleniumwire --include-package-data=qt_material --include-package-data=wordsegment --enable-plugin=pyside6 --include-qt-plugins=all --output-dir=E:/Project/Python/_APP/nts_videocreator  --follow-imports --noinclude-pytest-mode=nofollow --noinclude-setuptools-mode=nofollow --nofollow-import-to=tkinter --noinclude-unittest-mode=nofollow --windows-icon-from-ico=icon.ico NTSVideoCreator.py
